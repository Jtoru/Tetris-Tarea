...
import com.example.tetrisjtoru.constant.CellConstants
import com.example.tetrisjtoru.constant.FieldConstants
import com.example.tetrisjtoru.utils.array2dOfByte
...
class Game {
    ...
    var	currentState: String = Status.AWAITING_START.name
    //tamaño de 10*20
    ...
    fun	isGameOver(): Boolean {
        return currentState == Status.OVER.name
    }

    fun	isGameActive(): Boolean {
        return currentState == Status.ACTIVE.name
    }

    fun	isGameAwaitingStart(): Boolean {
        return currentState == Status.AWAITING_START.name
    }
    ...
    private fun resetField(temporalCellsOnly: Boolean = true) {
        for	(i in 0 until FieldConstants.ROW_COUNT.value) {
            (0 until FieldConstants.COLUMN_COUNT.value).filter { !temporalCellsOnly || field[i][it] == CellConstants.TEMPORAL.value }.forEach	{ field[i][it] = CellConstants.EMPTY.value }
        }
    }
    ...
    private	fun	persistCellData() {
        for	(i in 0 until field.size)	{
            for (j in 0 until field[i].size) {
                ...
                if (status == CellConstants.TEMPORAL.value) {
    ...
    fun startGame() {
        if (!isGameActive()) {
            currentState = Status.ACTIVE.name
            generateNextBlock()
        }
    }
    ...
    fun endGame() {
        score = 0
        currentState = Game.Status.OVER.name
    }

    private fun resetModel() {
        score = 0
        resetField(false)
        currentState = Status.AWAITING_START.name
