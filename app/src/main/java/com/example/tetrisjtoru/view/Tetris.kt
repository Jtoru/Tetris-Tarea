package com.example.tetrisjtoru.view
...
class Tetris : View {
    ...
    fun setModel(model: Game) {
        this.model = model
    }

    fun	setActivity(gameActivity: MainActivity)	{
        this.activity = gameActivity
    }

    fun setGameCommand(move: Game.Motions) {
        if (null !=	model && (model?.currentState == Game.Status.ACTIVE.name)) {
    ...
    private fun updateScores() {
        activity?.score?.text = "${model?.score}"
    ...
    private fun drawCell(canvas: Canvas, row: Int, col: Int) {
        ...
        if (CellConstants.EMPTY.value != cellStatus) {
            val	color = if (CellConstants.TEMPORAL.value == cellStatus) {
    ...
    private class ViewHandler(private val owner: Tetris) : Handler() {
